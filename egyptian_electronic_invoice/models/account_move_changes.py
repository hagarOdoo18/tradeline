...
def _partner_info(partner):
    if partner.country_id.code == 'EG':
        if partner.customer_type == 'Company':
            type = 'B'
            buyerId = partner.vat or ''

        else:
            type = 'P'
            buyerId = partner.national_id or ''

    else:
        type = 'F'
        buyerId = partner.passport_no or ''
...
def get_item_code(line):
    for rec in line:
        ...
        if rec.product_id.additional_code and rec.invoice_id.is_souq:
            ItemCode = str(rec.product_id.e_invoicing_code) + "/" + str(rec.product_id.additional_code)
        elif rec.product_id.additional_vendor_code:
            ItemCode = rec.product_id.additional_vendor_code
        else:
            ItemCode = rec.product_id.e_invoicing_code
    return ItemCode
...
class AccountMoveInherit(models.Model):
    ...
    def _compute_e_invoice_url(self):
        ...
        for rec in self:
            e_invoice_url = False
            if self.env.user.company_id.config_type == 'uat':
    ...
    @api.onchange('e_invoice_sent', 'e_invoice_status', 'state', 'e_invoice_uuid')
            ...
            if invoice.move_type in ('out_invoice', 'out_refund'):
                if invoice.state not in ('draft', 'cancel'):
                    if invoice.e_invoice_status in ('Draft', 'Cancelled'):
                        ...
                        main_content = {
                            'subject': 'error at E-invoice',
                            'author_id': self.env.user.partner_id.id,
                            'body_html': "error at Invalid Invoice " + str(invoice.invoice_origin),
                            'email_from': 'odooline21@gmail.com',
                            'email_to': 'Hagar.yehia@tradelinestores.net'
                        }
    ...
    def action_send_electronic_invoice(self):
        ...
        # Generate new token
        access_token, client_id, client_secret, apiBaseUrl = self.env.user.company_id.sudo().get_access_token()
        # Assign Headers
        ...
        if docs_length > self.env.user.company_id.max_invoice_per_time:
            raise UserError(_("It's not restricted to send maximum %s document per time,\n"
                              " and you are trying to send %s, Please reduce document to first number."
                              % (self.env.user.company_id.max_invoice_per_time, docs_length)))
        if docs_length > self.env.user.company_id.max_invoice_per_batch:
            docs_chunks = [self[x:x + self.env.user.company_id.max_invoice_per_batch]
                           for x in range(0, len(self), self.env.user.company_id.max_invoice_per_batch)]
        else:
            docs_chunks = [self]
        api_version = self.env.user.company_id.config_version
        for chunk in docs_chunks:
            for invoice in chunk:
                show_results = invoice.show_results
                internal_id = invoice.number
    ...
    def action_cancel_electronic_invoice(self):
        ...
        for invoice in self:
            ...
            if uuid:
                ...
                if not self.env.user.company_id.config_type:
                    raise ValidationError(
                        _("You must select Platform ENVIRONMENT in company %s first." % self.env.user.company_id.display_name))
    ...
    def _create_eta_attachment(self, datas):
        ...
        filename = ("%s-%s.pdf" % (self.number, self.e_invoice_uuid)).replace('/', '_')
    ...
    def _get_eta_personal_details(self, partner, order_total):
        ...
        if not partner.vat:
            ...
            elif type == 'P' and not id and order_total > 25000:
                raise ValidationError(
                    _("Missing National ID for partner [%s] cause order over 25K" % partner.display_name))
        if type == 'P' and order_total > 25000 and len(id) != 14:
        ...
        return {"address": {"country": partner.country_id.code,
                            ...
                            "buildingNumber":partner.street2 or '1',
    ...
    def _get_eta_invoice_lines(self):
        ...
        for line in self.invoice_line_ids.filtered(
            ...
            if line.currency_id and line.currency_id != EGP:
                ...
                #amountEGP = line.currency_id.compute(price_unit, EGP)
            ...
            taxes_res = line.invoice_line_tax_ids.compute_all(price_unit_wo_discount,
            ...
            uom = line.uom_id.eta_uom_id and line.uom_id.eta_uom_id.code or line.uom_id.name
            ...
            description =  str(line.product_id.display_name).replace('"','')
            description = description.replace("|",'')
            invoice_lines.append({
                "description": description or '',  # "Computerl"
                "itemType": line.product_id.code_type,  # "EGS"/"GS1"
                "itemCode": get_item_code(line),  # "EG-113317713-123456"
                "unitType": unitType,
                "quantity": quantity,
    ...
    def _get_eta_tax_totals(self, tax_lines):
        ...
        if tax_lines:
            ...
            for line in tax_lines:
                tax_type = line.tax_id.move_type_code_id.code or "T1"
                amount = self.currency_id.compute(line.amount_total, EGP)
                tax_totals.append({"taxType": tax_type,
                                   "amount": round(abs(amount), 5)})
        else:
            tax_type = self.env.user.company_id.move_type_code_id.code or "T1"
            tax_totals.append({"taxType": tax_type,
                               "amount": 0.00})
        return tax_totals
    ...
    def get_signature_value(self):
        ...
        for invoice in self:
            signature = False
            if self.env.user.company_id.signature_tool == 'python':
                if not invoice.e_invoice_canonical:
                    # Regenerate Json
                    invoice_params, env = invoice.action_generate_eta_json()
                static_sign_url = "%s" % self.env.user.company_id.signature_url
                user_pin = self.env.user.company_id.signature_pin or '52510779'
                token_label = self.env.user.company_id.signature_label or '52510779'
            ...
            elif self.env.user.company_id.signature_tool == 'c#':
                req_body = eval(invoice.e_invoice_json)
                invoice_params = json.dumps(req_body, indent=4, ensure_ascii=False).encode('utf8')
                try:
                    serialized = requests.post(url=self.env.user.company_id.signature_serializer,
                                               data=invoice_params, verify=False).content
                except Exception as e:
                    message = "Could Connect to %s due to connection error:\n %s" % (
                        self.env.user.company_id.signature_serializer, e)
                    logging.info(red + message + reset)
                    raise ValidationError(message)
                logging.info(green + "serialized: %s" % serialized + reset)
                try:
                    hashed = requests.post(url=self.env.user.company_id.signature_hash, data=serialized,
                                           verify=False).content
                except Exception as e:
                    message = "Could Connect to %s due to connection error:\n %s" % (
                        self.env.user.company_id.signature_hash, e)
                    logging.info(red + message + reset)
                    raise ValidationError(message)
    ...
    def action_generate_eta_json(self):
        env = self.env.user.company_id.config_type
        if not env:
            raise ValidationError(
                _("You must select Platform ENVIRONMENT in company %s first." % self.env.user.company_id.display_name))
        ...
        api_version = self.env.user.company_id.config_version
        # logging.info(yellow + "Date: %s " % self.invoice_date + reset)
        if self.send_today_for_einv:
            invoice_time = datetime.combine(datetime.today().date(), time.min)
        else:
            invoice_time = datetime.combine(self.invoice_date, time.min)
        invoice_time = datetime.combine(datetime.today().date(), time.min)
        ...
        taxpayerActivityCode = self.journal_id.eta_branch_code_id and self.journal_id.eta_branch_code_id.code or \
                               self.env.user.company_id.activity_code_id.code or "1061"
        issuer = self._get_eta_personal_details(self.env.user.company_id.partner_id, totalSalesAmount)
        issuer["address"]["branchID"] = "%s" % self.journal_id.eta_branch or 0
        receiver = self._get_eta_personal_details(self.partner_id, totalAmount)
        internal_id = self.number
        tax_total_lines = self._get_eta_tax_totals(tax_lines)
        now_time = fields.Datetime.now() - timedelta(hours=2)
        type = ''
        if self.inv_type:
            if self.inv_type == 'invoice':
                type = INV_TYP[self.move_type]
            else:
                type = 'D'
        else:
            type = INV_TYP[self.move_type]
        invoice_params = {
            ...
            "dateTimeIssued": invoice_time and invoice_time.replace(
                hour=now_time.hour, minute=now_time.minute, second=now_time.second).strftime('%Y-%m-%dT%H:%M:%SZ')
                              or fields.Datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ'),
            ...
            "purchaseOrderReference": self.e_invoice_po or "",
            "purchaseOrderDescription": self.e_invoice_po_desc or "",
            "salesOrderReference": self.e_invoice_so_ref or self.origin or "",
    ...
    def multi_invoice_sync(self):
        ...
        posted_invoices_ids = self.browse(active_ids).filtered(
            lambda i: not i.valid_flag and i.move_type in ('out_invoice', 'out_refund'))
    ...
    def multi_update_status(self):
        ...
        sent_invoices_ids = self.browse(self.env.context.get('active_ids')).filtered(
            lambda i: not i.valid_flag and i.state in ['open' ,'paid']and i.move_type in (
    ...
    def multi_invoice_cancel(self):
        ...
        canceled_invoices_ids = self.browse(self.env.context.get('active_ids')).filtered(
            lambda i: i.e_invoice_uuid and i.state == 'open' and
    ...
    def post(self):
        ...
        if auto_validate_documents:
            ready_documents = self.filtered(lambda l: l.move_type in ('out_invoice', 'out_refund') and
                                                      l.state == 'open' and not l.valid_flag)
            if ready_documents:
    ...
    def action_invoice_draft(self):
        ...
        return super(AccountMoveInherit, self).action_invoice_draft()
    ...
    def _check_sales_ref(self, invoice_params):
        ...
        if sales_list:
            ...
            invoice_params['salesOrderReference'] = sale_refs
        return invoice_params
    ...
    def generate_canonical(self):
        ...
        if response.get('status') in (200,202):
...
class AccountMoveLineInherit(models.Model):
    ...
    def _get_taxableItems(self, taxes_res):
        ...
        if taxes_res:
            for tax_line in taxes_res:
                ...
                if tax.move_type_code_id.code in ['T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12']:
                    totalTaxableFees += amount
                if self.currency_id:
                    # TODO: NEEDED to be changed
                    amount = self.currency_id.compute(amount, EGP)
                taxType = tax.move_type_code_id.code or self.env.user.company_id.move_type_code_id.code
                subType = tax.sub_type_code_id.code or self.env.user.company_id.sub_type_code_id.code
                rate = abs(tax.amount)
                taxableItems.append({
                    "taxType": taxType,
            ...
            taxableItems = [{"taxType": self.env.user.company_id.move_type_code_id.code,
                             "amount": 0.00,
                             "subType": self.env.user.company_id.sub_type_code_id.code,
                             "rate": 0.00}]
    ...
    def get_price_unit(self):
        ...
        if self.invoice_line_tax_ids:
            taxes_included = self.invoice_line_tax_ids.filtered(lambda tx: tx.price_include)
            for tax in taxes_included:
                tax_line = tax.compute_all(price_unit_wo_discount,
