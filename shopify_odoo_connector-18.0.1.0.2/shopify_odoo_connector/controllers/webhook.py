...
class WebHook(http.Controller):
    ...
    def get_webhook_url(self):
        ...
        try:
            if not shopify_instance_id.sudo().is_exporting:
                ...
                product_id = request.env['product.template'].with_user(SUPERUSER_ID).search(
                    [
                        ('shopify_sync_ids.shopify_product', '=', data.get('id')),
                        ('shopify_sync_ids.instance_id', '=',shopify_instance_id.id),
                        ('company_id', 'in',
    ...
    @http.route('/update_products', type='json', auth='none',
                methods=['POST'],
                csrf=False)
    ...
    @http.route('/delete_products', type='json', auth='public',
                methods=['POST'],
                csrf=False)
    ...
    def get_webhook_customer_url(self, *args, **kwargs):
        ...
        try:
            if not shopify_instance_id.sudo().is_exporting:
                ...
                if (not data['first_name'] and not data['last_name'] and
                        data['email']):
    ...
    @http.route('/draft_orders', type='json', auth='none',
    ...
    def get_webhook_order_url(self, *args, **kwargs):
        ...
        try:
            if not shopify_shop.sudo().is_exporting:
                exist_order = request.env['shopify.sync'].with_user(
                    SUPERUSER_ID).search(
                    [('shopify_order_ref', '=', data['id'])
                     ])
                if not exist_order:
                    shop_name = request.httprequest.headers.get(
                        'X-Shopify-Shop-Domain')
                    if shop_name == 'nuthousecuracao.myshopify.com':
                        shopify_team = (request.env.ref(
                            ...
                            'sales_team_customers_shopify').id)
                    else:
                        shopify_team = request.env.ref(
                            'shopify_odoo_connector.'
                            'sales_team_business_shopify').id
                    shopify_shop = request.env[
                        'shopify.configuration'].with_user(SUPERUSER_ID).search(
                        [('shop_name', 'like', shop_name)
                         ], limit=1)
                    ...
                    if not partner_id:
                        ...
                        partner_.shopify_sync_ids.sudo().create({
                            ...
                            'shopify_customer_ref': data['customer'].get('id'),
                    ...
                    for item in data['line_items']:
                        ...
                        if not product_id:
                            ...
                            product_.shopify_sync_ids.sudo().create({
                                'instance_id': shopify_shop.id,
                                'shopify_product': product_shopify_id,
                                'product_prod_id': product_id,
                            })
                        discount_per = 0
                        if data.get("applied_discount"):
                            price_subtotal = float(item['quantity']) * float(
                                item['price'])
                            discount_per = (float(
                                data.get("applied_discount").get(
                                    "amount")) / len(
                                data['line_items']) / price_subtotal) * 100
                        vals.append((0, 0,
                                     {'product_id': product_id,
                                      ...
                                      'tax_id': [
                                          (6, 0,
                                           tax_name.ids)] if tax_name else
                                      False,
                                      }))
                    if data['shipping_line']:
                        product_id = request.env.ref(
                            'shopify_odoo_connector.'
                            'product_shopify_shipping_cost')
                        vals.append((0, 0,
                                     {'product_id': product_id.id,
                                      'product_uom_qty': 1,
                                      'price_unit': data['shipping_line'][
                                          'price'],
                                      'tax_id': False,
                                      }))
                    so = request.env['sale.order'].with_user(
                        SUPERUSER_ID).create(
        ...
        try:
            if not shopify_shop.sudo().is_exporting:
                ...
                sale_order = request.env['sale.order'].with_user(
                    SUPERUSER_ID).search(
                    [('shopify_sync_ids.shopify_order_ref', '=', data['id']),
                     (
                ...
                if data['tax_lines']:
                    tax_group_id = request.env[
                        'account.tax.group'].with_user(SUPERUSER_ID).search(
                        [
                            ('name', '=', data['tax_lines'][0]['title'])
                        ])
                    tax_name = request.env[
                        'account.tax'].with_user(SUPERUSER_ID).search(
                        [
                            ('amount', '=', data['tax_lines'][0]['rate'] * 100),
                            ('tax_group_id', '=', tax_group_id.id),
                            ('type_tax_use', '=', 'sale'),
                            (
                                'company_id', 'in',
                    ...
                    if not tax_name:
                        ...
                        tax_name = request.env[
                            'account.tax'].with_user(SUPERUSER_ID).create(
                            {
                                'name': data['tax_lines'][0]['title'] + str(
                                    data['tax_lines'][0]['rate'] * 100) + '%',
                                'type_tax_use': 'sale',
                                'amount_type': 'percent',
                                'amount': data['tax_lines'][0]['rate'] * 100,
                                'tax_group_id': tax_group_id.id,
                ...
                for item in data['line_items']:
                    ...
                    if discount:
                        discount_per = (float(discount.get("amount")) / len(
                            data['line_items']) / float(
                            item['quantity']) * float(
                            item['price'])) * 100
                    vals.append((0, 0,
                                 {'product_id': product_id,
                ...
                sale_order.shopify_order_ref = data['id']
                if data['status']:
                    if sale_order:
                        line.sudo().write({
        ...
        try:
            if not shopify_instance_id.sudo().is_exporting:
                order_id = request.env['sale.order'].with_user(
                    SUPERUSER_ID).search(
                    [
                        ('shopify_sync_ids.shopify_order_ref', '=',
                         request.get_json_data()['id']),
                        ('shopify_sync_ids.instance_id', '=',
                         shopify_instance_id.id)
                    ])
    ...
    def get_webhook_create_order_url(self, *args, **kwargs):
        ...
        try:
            if not shopify_shop.sudo().is_exporting:
                ...
                if shop_name == 'nuthousecuracao.myshopify.com':
                    shopify_team = (request.env.ref(
                        'shopify_odoo_connector.sales_team_customers_shopify').
                                    id)
                else:
                    shopify_team = request.env.ref(
                ...
                if data['tax_lines']:
                    ...
                    taxes = data['tax_lines'][0]['rate'] * 100
                    tax_name = request.env[
                        'account.tax'].with_user(SUPERUSER_ID).search(
                        [
                            ('amount', '=', taxes),
                ...
                for item in data['line_items']:
                    ...
                    discount_per = 0
                    if data.get("discount_applications"):
                        for items in data.get("discount_applications"):
                            amount = items.get("value")
                            dis_amt = float(amount)
                            line_amt = dis_amt / len(data['line_items'])
                        ...
                        if payment_id.payment_status == 'paid':
                            ...
                            if sale_order.invoice_ids:
                                ...
                                invoice_pay = request.env[
                                    ...
                                    SUPERUSER_ID).with_context(
                                    active_model='account.move',
                                    active_ids=sale_order.
                                    invoice_ids.id).create({})
                                invoice_pay._create_payments()
                return {'Message': 'Success'}
        except Exception:
            request.env['log.message'].sudo().create({
        ...
        try:
            if not shopify_shop.sudo().is_exporting:
                ...
                if request.get_json_data()['tax_lines']:
                    ...
                    taxes = request.get_json_data()['tax_lines'][0][
                                'rate'] * 100
                    tax_name = request.env[
                        'account.tax'].with_user(SUPERUSER_ID).search(
                        [
                                ...
                                if not product_id:
                                    product_id = request.env[
                                        ...
                                        SUPERUSER_ID).create(
                                        {
                                            'name': rec['title'],
                                            'company_id':
                                                shopify_instance_id.company_id.
                                                id,
                                        }).id
                                    product_ = request.env[
                                        'product.product'].with_user(
    ...
    @http.route('/order_payment', type='json', auth='none',
                methods=['POST'], csrf=False)
    def get_webhook_order_payment_url(self, *args, **kwargs):
        ...
        try:
            if not shopify_instance_id.sudo().is_exporting and \
                    request.get_json_data()['id']:
                ...
                if order_id:
                    ...
                    request.env['shopify.payment'].with_user(
                        SUPERUSER_ID).create(
                        {
                            'shopify_order_ref': request.get_json_data()['id'],
            ...
            if not shopify_instance_id.sudo().is_exporting:
                ...
                for rec in data["refund_line_items"]:
                    ...
                    if record['tax_lines']:
                        ...
                        tax_line = tax_lines[item]
                        price = float(tax_line['price']) + float(
                            record['price'])
                        item += 1
                    else:
                        price = record['price']
        ...
        try:
            if not shopify_instance_id.sudo().is_exporting:
                ...
                event_type = request.httprequest.headers.get(
                    'X-Shopify-Topic')
                if event_type == 'collections/create':
                    collection = request.env['shopify.collection'].sudo().search([
                        ('collection', '=', data['id']),
                        ('shopify_instance_id', '=', shopify_instance_id.id),
                    ])
                    if not collection:
                        request.env['shopify.collection'].sudo().create(
                            {
                                'name': data['title'],
                                'collection': data['id'],
                                'shopify_instance_id': shopify_instance_id.id,
                        ...
                        request.env['shopify.collection'].sudo().write(
                            {
                                'name': data['title'],
                            }
                        )
                elif event_type == 'collections/update':
                    collection = request.env['shopify.collection'].sudo().search([
                        ('collection', '=', data['id']),
                        ('shopify_instance_id', '=', shopify_instance_id.id),
                    ])
                ...
                elif event_type == 'collections/delete':
