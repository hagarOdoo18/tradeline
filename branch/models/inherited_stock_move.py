...
class StockMove(models.Model):
    ...
    def check_picking(self, pickings, move):
        for picking in pickings:
            if picking.picking_type_id.code != 'internal' and picking:

                if picking.partner_id.id != move.partner_id.id or picking.origin != move.origin:
                    return picking
                else:
                    return 'same picking', picking
            elif picking.picking_type_id.code == 'internal' and picking:
                for move_picking in picking.move_ids_without_package:
    ...
    #     for move in self:
    #         recompute = False
    #         pickings = move._search_picking_for_assignation()
    #         picking = self.check_picking(pickings, move)
    #         if picking:
    #             if len(picking) == 1:
    #                 picking.write({
    #                     'partner_id': False,
    ...
    #             else:
    #                 move.write({'picking_id': picking[1].id})
    #                 move._assign_picking_post_process(new=picking)
    #         else:
    #             recompute = True
    #             old_picking = self.env['stock.picking'].search([('origin', '=', move.origin)])
    #
    #             if old_picking and  picking:
    #                 if old_picking.note and old_picking.origin:
    #                     picking.note = old_picking.note
    #                     picking.note += ' / ' + old_picking.origin
    #                 elif old_picking.note:
    #                     picking.note = old_picking.note
    #                 else:
    #                     picking.note = old_picking.origin
    #             else:
    ...
    def _get_new_picking_values(self):
        ...
        if not  vals['branch_id']:
            vals['branch_id'] =self.env['stock.location'].sudo().browse(vals['location_dest_id']).branch_id.id
