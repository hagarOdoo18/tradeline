  ...
  async loadLatestProductQtyFromDB() {
    ...
    if (products && products.length > 0) {
      products.forEach((product) => {
        const localProduct = productById[product.id];
        if (localProduct) {
          localProduct.qty_available = product.qty_available;
          localProduct.virtual_available = product.virtual_available;
        }
      });
    }
    } catch (error) {
