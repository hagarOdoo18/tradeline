...
patch(SearchModel.prototype, {
    async load(config) {
        ...
        const preferredDateFilterIds = new Set(
            searchItems.filter(isNativeTradelineDateFilter).map((item) => item.id)
        );
        const firstDateFilterId = searchItems.find((item) => item?.type === "dateFilter")?.id;

        this.searchItems = searchItems.filter((item) => {
            if (isLegacyTimeFilter(item)) {
                return false;
            }
            ...
            return firstDateFilterId ? item.id === firstDateFilterId : false;
        });
