...
def _dump_context(context_dict):
    return repr(context_dict or {})
...
def _search_view_has_field(search_view, field_name):
    arch = (search_view.arch_db or "").strip()
    if not arch:
        return False
    try:
        ...
        return False
    return bool(root.xpath(f"//search/field[@name='{field_name}']"))
...
def _collect_reporting_actions(env):
    ...
    for xmlid in EXPLICIT_TIME_ACTION_XMLIDS:
    ...
    for menu in menus:
        ...
        if "report" not in complete_name and "reporting" not in complete_name:
            continue
    ...
    actions |= action_model.search([("res_model", "=", "account.invoice.report")])
...
def _merge_action_context(action, enable_groupby=False, enable_branch_alpha=False, enable_native_time=False):
    ...
    if changed:
        action.context = _dump_context(parsed_context)
    return changed
...
def _migrate_favorite_context(record):
    ...
    compare_mode = parsed_context.get("tradeline_time_compare")
    native_filter_name = _map_legacy_range_to_native_filter(model_name, based_on, range_key)

    changed = False
    unmapped = False

    if native_filter_name and compare_mode in (None, False, "none"):
        ...
        unmapped = True
        if not parsed_context.get("tradeline_time_legacy_mode"):
    ...
    if changed:
        record.context = _dump_context(parsed_context)
...
def _migrate_legacy_favorites(env, models_to_scan):
    ...
    _logger.info(
        "tradeline_time_ranges migration: scanned=%s migrated=%s unmapped=%s",
        len(favorites),
        migrated,
        unmapped,
    )
...
def _inventory_reporting_actions(actions):
    for action in actions:
        ...
        _logger.info(
            "tradeline_time_ranges inventory action_id=%s name=%s model=%s action_search_view_id=%s effective_search_view_id=%s date_fields=%s",
            action.id,
            action.name,
            action.res_model,
...
def sync_native_time_ranges(env):
    ...
    for action in reporting_actions:
        ...
        if not search_view:
            continue
        if search_view.id in processed_search_view_ids:
            continue
