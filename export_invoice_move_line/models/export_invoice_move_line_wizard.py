...
class ExportInvoiceMoveLineWizard(models.TransientModel):
    ...
    def action_export_excel(self):
        ...
        if self.env.user.has_group('export_invoice_move_line.group_export_invoice_move_line_admin'):
            headers = ['Date','Branch','Ref','Credit','Opportunity','Customer Name','Customer Mobile','Customer Phone','Customer type',
                       'Product Category','Family','UPC','Item Code','Description' ,'Quantity','Serial','Unit Cost',
                      'Unit Price', 'Discount (%)','Discount Reason','Amount signed','Price Total Signed','Invoice Amount signed','Total Cost' ,'Invoice Price Total Signed','Payment Journals'
            ...
            for line in invoice_lines:
                ...
                cost = line.sudo().product_id.standard_price
                ...
                sheet.write(row, 8, line.move_id.partner_id.company_type or '')
                sheet.write(row, 9, line.product_id.categ_id.name or '')
                sheet.write(row, 10, line.product_id.product_tmpl_id.family_id.name or '')
                sheet.write(row, 11, line.product_id.default_code or '')
                sheet.write(row, 12, line.product_id.barcode or '')
                sheet.write(row, 13, line.product_id.display_name or '')
                sheet.write(row, 14, line.quantity if line.move_id.move_type != 'out_refund' else line.quantity *-1 or '')
                sheet.write(row, 15, serials or '')
                sheet.write(row, 16, float(cost) or 0)
                sheet.write(row, 17, float(line.price_unit) or 0)
                sheet.write(row, 18, line.discount or 0)
                sheet.write(row, 19, line.discount_reason_id.name if line.discount >0 else ''  )
                sheet.write(row, 20, line.price_subtotal if line.move_id.move_type != 'out_refund' else  line.price_subtotal*-1 or 0)
                sheet.write(row, 21, line.price_total  if line.move_id.move_type != 'out_refund' else  line.price_total*-1 or 0)
                sheet.write(row, 22, amount_untaxed_signed if line.move_id.id not in invoices  else 0)
                sheet.write(row, 23, float(cost * line.quantity) if line.move_id.move_type != 'out_refund' else  float(line.product_id.standard_price * line.quantity) *-1 or 0)
                sheet.write(row, 24,amount_total_signed if line.move_id.id not in invoices  else 0)
                sheet.write(row, 25, payment_journals if line.move_id.id not in invoices  else '')
                sheet.write(row, 26, amount if line.move_id.id not in invoices  else 0)
                sheet.write(row, 27, line.move_id.sales_rep_id.name or '')
                sheet.write(row, 28, line.move_id.reference_number or '')
                sheet.write(row, 29, line.product_id.vendor_id.name or '')
                sheet.write(row, 30, line.product_point or '')
                sheet.write(row, 31, line.move_id.channel_id.name or '')
                sheet.write(row, 32, line.move_id.currency_id.name or '')
            ...
            headers = ['Date', 'Branch', 'Ref', 'Credit', 'Opportunity', 'Customer Name', 'Customer Mobile',
                       ...
                       'Unit Cost',
                       'Unit Price', 'Discount (%)','Discount Reason','Amount signed','Price Total Signed', 'Invoice Amount signed', 'Total Cost', 'Invoice Price Total Signed',
                       'Payment Journals'
            ...
            for line in invoice_lines:
                ...
                sheet.write(row, 28, line.move_id.reference_number or '')
                sheet.write(row, 29, line.product_id.vendor_id.name or '')
                sheet.write(row, 30, line.move_id.channel_id.name or '')
                sheet.write(row, 31, line.move_id.currency_id.name or '')
                invoices.append(line.move_id.id)

                row += 1
        elif self.env.user.has_group('export_invoice_move_line.group_export_invoice_move_line_accounting'):
            headers = ['Date', 'Branch', 'Ref', 'Credit', 'Opportunity', 'Customer Name', 'Customer Mobile',
                       'Customer Phone','Customer type','Address','Tax ID OR National ID','Employees','Devices','Tags','Apple Store ID',
                       'Product Category', 'Family', 'UPC', 'Item Code', 'Description', 'Quantity', 'Serial',
                       'Unit Cost',
                       'Unit Price', 'Discount (%)','Discount Reason','Amount signed','Price Total Signed', 'Invoice Amount signed', 'Total Cost', 'Invoice Price Total Signed',
                       'Payment Journals'
            ...
            for line in invoice_lines:
                ...
                sheet.write(row, 8, partner.company_type or '')
                sheet.write(row, 9, partner.street or '')
                sheet.write(row, 10, partner.vat or '')
                sheet.write(row, 11, dict(partner._fields['company_size'].selection).get(partner.company_size) or '')
                sheet.write(row,12, partner.company_device or '')
                sheet.write(row, 13, ", ".join(partner.category_id.mapped("name")) or '')
                sheet.write(row, 14,  line.move_id.branch_id.apple_store_id or '')
                sheet.write(row, 15, line.product_id.categ_id.name or '')
                sheet.write(row, 16, line.product_id.product_tmpl_id.family_id.name or '')
                sheet.write(row, 17, line.product_id.default_code or '')
                sheet.write(row, 18, line.product_id.barcode or '')
                sheet.write(row, 19, line.product_id.display_name or '')
                sheet.write(row, 20, line.quantity  if line.move_id.move_type != 'out_refund' else line.quantity *-1 or '')
                sheet.write(row, 21, serials or '')
                sheet.write(row, 22, float(cost) or 0)
                sheet.write(row, 23, float(line.price_unit) or 0)
                sheet.write(row, 24, line.discount or 0)
                sheet.write(row, 25,line.discount_reason_id.name if line.discount >0 else '')
                sheet.write(row, 26, line.price_subtotal  if line.move_id.move_type != 'out_refund' else  line.price_subtotal*-1 or 0)
                sheet.write(row, 27, line.price_total  if line.move_id.move_type != 'out_refund' else  line.price_total*-1 or 0)
                sheet.write(row, 28, amount_untaxed_signed if line.move_id.id not in invoices  else 0)
                sheet.write(row, 29,  float(cost * line.quantity) if line.move_id.move_type != 'out_refund' else  float(line.product_id.standard_price * line.quantity) *-1  or 0)
                sheet.write(row, 30,amount_total_signed if line.move_id.id not in invoices  else 0)
                sheet.write(row, 31, payment_journals if line.move_id.id not in invoices  else '')
                sheet.write(row, 32, amount if line.move_id.id not in invoices  else 0)
                sheet.write(row, 33, line.move_id.sales_rep_id.name or '')
                sheet.write(row, 34, line.move_id.reference_number or '')
                sheet.write(row, 35, line.product_id.vendor_id.name or '')
                sheet.write(row, 36, line.move_id.channel_id.name or '')
                sheet.write(row, 37, line.move_id.currency_id.name or '')
                invoices.append(line.move_id.id)
            ...
            headers = ['Date', 'Branch', 'Ref', 'Credit', 'Opportunity', 'Customer Name', 'Customer Mobile',
                       'Customer Phone','Customer type',
                       'Product Category', 'Family', 'UPC', 'Item Code', 'Description', 'Quantity', 'Serial',
                       'Unit Price', 'Discount (%)','Discount Reason', 'Amount signed','Price Total Signed','Invoice Amount signed', 'Invoice Price Total Signed',
                       'Payment Journals'
            ...
            for line in invoice_lines:
                ...
                sheet.write(row, 14, line.quantity if line.move_id.move_type != 'out_refund' else line.quantity *-1 or '')
                sheet.write(row, 15, serials or '')
                sheet.write(row, 16, line.price_unit or '')
                sheet.write(row, 17, line.discount or '')
                sheet.write(row, 18, line.discount_reason_id.name if line.discount >0 else '')
                sheet.write(row, 19, line.price_subtotal  if line.move_id.move_type != 'out_refund' else  line.price_subtotal*-1 or 0)
                sheet.write(row, 20, line.price_total if line.move_id.move_type != 'out_refund' else  line.price_total*-1 or 0)
                sheet.write(row, 21, amount_untaxed_signed if line.move_id.id not in invoices  else 0)
                ...
                sheet.write(row, 27, line.product_point or '')
